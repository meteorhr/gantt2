...
export class DcmaChecksComponent {
    ...
    try {
      const [check1, check2, check3, check4, check5, check6, check7, check8, check9, check10, check11, check12, check13, check14] = await Promise.all([
        this.svc.analyzeCheck1(this.projId(), {
          ...
          ignoreLoEAndHammockLinksInLogic: true, // <-- строгий режим
  ...
  private buildRows() {
    type DcmaRowT = { check: number; metric: string; description: string; percent: number | null; passed: boolean; result: any };
    const rows: DcmaRowT[] = [];
    const push = (check: number, metric: string, description: string, percent: number | null, passed: boolean, result: any) => {
      rows.push({ check, metric, description, percent, passed, result });
    };
    const r1 = this.r1(); if (r1) push(1, 'Logic', `Missing any: ${r1.uniqueMissingAny}/${r1.totalEligible}`, r1.percentMissingAny, r1.percentMissingAny <= 5, r1);
    const r2 = this.r2(); if (r2) push(2, 'Leads', `Lead links: ${r2.leadCount}/${r2.totalRelationships}`, r2.leadPercent, r2.leadCount === 0, r2);
    const r3 = this.r3(); if (r3) push(3, 'Lags', `Lag links: ${r3.lagCount}/${r3.totalRelationships}`, r3.lagPercent, r3.lagPercent <= 5, r3);
    const r4 = this.r4(); if (r4) push(4, 'Relationship Types', `FS: ${r4.countFS} (of ${r4.totalRelationships})`, r4.percentFS, r4.percentFS >= 90, r4);
    const r5 = this.r5(); if (r5) push(5, 'Hard Constraints', `Hard: ${r5.hardCount}/${r5.totalWithConstraints}`, r5.hardPercent, r5.hardPercent <= 5, r5);
    const r6 = this.r6(); if (r6) push(6, 'High Float', `High TF: ${r6.highFloatCount}/${r6.totalEligible}`, r6.highFloatPercent, r6.highFloatPercent <= 5, r6);
    const r7 = this.r7(); if (r7) push(7, 'Negative Float', `Neg TF count: ${r7.negativeFloatCount}`, null, !r7.hasNegativeFloat, r7);
    const r8 = this.r8(); if (r8) push(8, 'High Duration', `>44d remain: ${r8.highDurationCount}/${r8.totalEligible}`, r8.highDurationPercent, r8.highDurationPercent <= 5, r8);
    const r9 = this.r9(); if (r9) push(9, 'Invalid Dates', `9a: ${r9.invalidForecastCount} • 9b: ${r9.invalidActualCount}`, null, !r9.hasInvalidDates, r9);
    const r10 = this.r10(); if (r10) push(10, 'Resources', `No resources: ${r10.withoutResourceCount}/${r10.totalEligible}`, r10.percentWithoutResource, r10.withoutResourceCount === 0, r10);
    const r11 = this.r11(); if (r11) push(11, 'Missed Tasks', `Missed: ${r11.missedCount}/${r11.totalCompleted}`, r11.missedPercent, r11.missedPercent <= 5, r11);
    const r12 = this.r12(); if (r12) push(12, 'Critical Path Test', `Single chain & ends at PF: ${r12.isSingleChain && r12.reachedProjectFinish ? 'OK' : 'Issue'}`, null, !!r12.testPassLikely, r12);
    const r13 = this.r13(); if (r13) push(13, 'CPLI', `CPL: ${r13.criticalPathLengthDays} • PTF: ${r13.projectTotalFloatDays}`, r13.cpli ?? null, !!r13.cpliWithin5pct, r13);
    const r14 = this.r14(); if (r14) push(14, 'BEI', `Planned/Actual: ${r14.plannedToComplete}/${r14.actuallyCompleted}`, r14.bei ?? null, !!r14.beiWithin95pct, r14);
    this.rows.set(rows);
  }
}