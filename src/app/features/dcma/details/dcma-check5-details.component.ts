    ...
    <mat-tab label="{{ 'dcma.summary' | transloco }}">
      <div class="c1-summary"
        ...
        <p class="c1-summary__line">
          ...
          {{ row.result?.hardCount }} / {{ row.result?.totalWithConstraints }}
        </p>
      </div>
    </mat-tab>

    @if (row.result?.details?.hardList?.length) {
        ...
        <div class="vtable">
          ...
          <cdk-virtual-scroll-viewport class="v-viewport"
            ...
            <table class="vtable__body mat-elevation-z1">
              <tbody>
                <tr *cdkVirtualFor="let i of row.result.details.hardList; trackBy: trackHard">
...
export class DcmaCheck5DetailsComponent {
  ...
  trackHard = (_: number, i: any) =>
    i?.id ?? `${i?.task_id || i?.task_code || ''}|${i?.cstr_type || ''}|${i?.cstr_date || ''}`;
