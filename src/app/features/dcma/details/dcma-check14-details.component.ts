...
@Component({
  ...
  <mat-tab-group mat-stretch-tabs="false" mat-align-tabs="start" (selectedTabChange)="onTabChange()">
    ...
    @if (row.result?.details?.plannedButNotCompleted?.length) {
      <mat-tab label="{{ 'dcma.c14.plannedNotCompleted' | transloco }} ({{ row.result.details.plannedButNotCompleted.length }})">
        <div class="vtable">
          ...
          <cdk-virtual-scroll-viewport
            ...
            <table class="vtable__body mat-elevation-z1">
              <tbody>
                <tr *cdkVirtualFor="let i of row.result.details.plannedButNotCompleted; trackBy: trackC14Planned">
    ...
    @if (row.result?.details?.completedAheadOfPlan?.length) {
      <mat-tab label="{{ 'dcma.c14.completedAhead' | transloco }} ({{ row.result.details.completedAheadOfPlan.length }})">
        <div class="vtable">
          <table class="vtable__head">
            <thead>
              <tr>
          ...
          <cdk-virtual-scroll-viewport
            ...
            <table class="vtable__body mat-elevation-z1">
              <tbody>
                <tr *cdkVirtualFor="let i of row.result.details.completedAheadOfPlan; trackBy: trackC14Ahead">
                  <td>{{ i.task_code || i.task_id }}</td>
                  <td>{{ i.task_name }}</td>
...
export class DcmaCheck14DetailsComponent {
