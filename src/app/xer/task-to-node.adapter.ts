...
type PredRow = { task_id: number; pred_task_id: number };
...
export interface WbsBuildOptions {
  ...
  baselineSource?: BaselineSource;       // <-- НОВОЕ: откуда брать baseline у задач
  debug?: boolean;
}

export function buildWbsTaskTree(doc: XERDocument, opts?: WbsBuildOptions): Node[] {
  // -------- 1) читаем WBS и строим узлы-зародыши --------
  ...
  // -------- 2) возможные переопределения родителей --------
  ...
  // -------- 3) предшественники для dependency --------
  const preds = buildPredecessorMap(doc);

  // Источник baseline по умолчанию — target (как в большинстве XER)
  const baselineSource: BaselineSource = opts?.baselineSource ?? 'target';

  // -------- 4) TASK -> Node и прикрепляем к WBS --------
  ...
  for (const t of tasks) {
    ...
    // baseline по выбранному источнику
    const baselineStart =
    ...
    const complete = clamp0to100(numberOrNull(t.phys_complete_pct)) ?? undefined;
  ...
  // -------- 5) линковка WBS→WBS (по уже исправленным parentWbsId) --------
  ...
  // -------- 6) корневые WBS сортируем по seq_num, затем по имени --------
  const rootWbsNodes = [...wbsMap.values()].filter(w => {
    const hasParent = w.parentWbsId != null && wbsMap.has(w.parentWbsId);
    return !hasParent;
  });
  rootWbsNodes.sort((a, b) => (a.seq - b.seq) || a.name.localeCompare(b.name));

  // финализуем в Node[] (OPC-порядок: сначала задачи, затем WBS)
  const roots: Node[] = [];
...
function buildPredecessorMap(doc: XERDocument): Map<number, string[]> {
  ...
  const rows = getRows<PredRow>(doc, 'TASKPRED');
  for (const r of rows) {
...
function pickDate<T extends Record<string, XERScalar>>(row: T, key: keyof T): Date | string | null | undefined {
  const v = row[key];
